 ...
 */
public class OldCalendarFetcher extends Observable implements Runnable {
    private final static String TAG = "OldCalendarFetcher";

    private Context ctx = null;

    public OldCalendarFetcher() {
    ...
    private volatile OldCalendarResults oldCalendarResults = null;
     ...
     * automatically when the OldCalendarResults class is created.
     */
    public OldCalendarResults getContent() {
        return oldCalendarResults;
    }
     ...
     */
    private OldCalendarResults loadContent() {
        // local copy; don't overwrite the class variable until we're done!
        OldCalendarResults cr = new OldCalendarResults();
        ...
        if(iCursor != null) {
            if (iCursor.moveToFirst()) {
                do {
                    OldCalendarResults.Instance instance = new OldCalendarResults.Instance();
                    int i = 0;
...
//                Log.v(TAG, "visible instances found: " + instance.toString());
        ...
        if(cr.instances.size() > 1) {
            ...
            Collections.sort(cr.instances, new Comparator<OldCalendarResults.Instance>() {
                @Override
                public int compare(OldCalendarResults.Instance lhs, OldCalendarResults.Instance rhs) {
